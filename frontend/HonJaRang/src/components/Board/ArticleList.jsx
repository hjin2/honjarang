...
export default function AricleList() {
  ...
  const fetchArticles = useCallback(() =>{
    axios.get(`${API.POST}`,{params:{page:currentPage, keyword:keyword}, headers})
    .then((res)=>{
  ...
  useEffect(()=>{
    axios.get(`${API.POST}/page`,{params:{size:15, keyword:keyword}, headers})
      .then((res)=>{
  ...
  return (
    ...
    <div className='flex justify-between mt-5'>
      <form action="" className="space-x-2" onSubmit={search}>
        <input className="border border-gray2 focus:outline-main2 h-10 p-2"
        type="text" placeholder="검색어" onChange={handleKeyword}/>
        <button>
