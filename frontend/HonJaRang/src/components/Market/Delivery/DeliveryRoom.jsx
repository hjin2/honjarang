...
export default function DeliveryRoom(roomData) {
  ...
  const imageToShow = store_image.includes('blogfiles')
    ? defaultImage
    : store_image;
  ...
  return (
    <div>
      <div className="border-2 border-gray1 p-2 rounded-lg">
        ...
        <div className="text-sm text-main5 my-1">
          목표까지{' '}
          {target_min_price - current_total_price > 0
            ? (target_min_price - current_total_price).toLocaleString()
            : 0}
          원
        </div>
