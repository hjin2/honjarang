...
export default function Refund({ modalState, setModalState }) {
  ...
  const handleRefund = () => {
    ...
    axios
      .put(`${API.USER}/withdraw`, data, { headers })
  ...
  return (
    <div className="space-y-4">
      ...
      <input
        type="number"
        placeholder="환급할 포인트를 입력해주세요."
        className="h-8 text-xs w-full"
        onChange={handleInputPoint}
      />
      <div>환급 후 포인트</div>
      <div className="h-8 border rounded-lg">
        {Number(point) - Number(inputPoint)}P
      </div>
      <hr />
      <div>환급 계좌 선택</div>
      <select
      ...
      <div className="text-xs">
        계좌 번호 오기입으로 인한 미환급은 책임지지 않습니다. 계좌번호를 반드시
        확인해 주세요.
      </div>
      <div className="flex justify-between">
        <button
          type="button"
          className="main1-button w-24"
