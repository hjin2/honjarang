...
export default function Edit({ modalState, setModalState }) {
  ...
  const axiosimageUpload = (formData) =>{
    axios.post(`${API.USER}/profile-image`,formData,{headers:{"Authorization":`Bearer ${token}`, "Content-Type":"multipart/form-data"}})
    .then((res)=>{
      console.log(res)
  ...
  const axiosEditInfo = (data) => {
    axios.put(`${API.USER}/users`, data, { headers:{'Authorization': `Bearer ${token}`, "Content-Type":"Application/json"} })
      .then((res)=>{
          console.log(res)
