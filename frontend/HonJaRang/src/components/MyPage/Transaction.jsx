...
export default function Transaction({id, isMe}) {
  ...
  const headers = {"Authorization" : `Bearer ${token}`}
	...
	useEffect(()=>{
    axios.get(`${API.USER}/page-joined-transaction`, {params:{size:4}, headers})
      .then((res)=>{
	...
	useEffect(()=>{
    axios.get(`${API.USER}/transaction-participating`, {params:{page:joinCurrentPage, size:4},headers})
      .then((res) => {
        console.log(res.data)
  ...
  useEffect(() => {
    axios.get(`${API.USER}/page-transaction/${id}`,{params:{size:4},headers})
      .then((res) =>{
        console.log(res.data)
	...
	useEffect(() => {
    axios.get(`${API.USER}/transaction-writer/${id}`,{params:{size:4,page:writeCurrentPage},headers})
      .then((res) => {
        console.log(res.data)
