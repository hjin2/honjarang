...
export default function DeliveryDetail() {
  ...
  useEffect(() => {
    // 공동 배달 상세
    axios.get(`${API.DELIVERIES}/${id}`,  {headers})
    .then((res) => {
      setDetail(res.data)
    ...
    // 메뉴 목록 조회
    axios.get(`${API.DELIVERIES}/${id}/menus`, {headers})
    .then((res) => {
  ...
  const deleteDelivery = () =>{
    axios.delete(`${API.DELIVERIES}/${id}`,{headers})
      .then((res)=>{
        console.log(res)
        navigate('/market')
      })
        ...
        <div className="menu-container overflow-y-scroll max-h-96 border rounded-md px-10">
            {menuList.map((menu) => (
              <div key={menu.id} className="flex justify-between items-center space-x-4 my-2">
                ...
                {timeDiff > 0 ? (
                  <div>
                    <button onClick={() => onModalOpen(menu)} className="hover:text-main2">
      ...
      <div className="border rounded-lg max-w-2xl mx-auto mt-5 mb-10 pb-3 p-5 space-y-5 flex flex-col items-center text-center">
        {timeDiff > 0 ? (
          <div>
            ...
            <div className="text-main2">목표까지 {detail.target_min_price - detail.current_total_price >0 ? (detail.target_min_price - detail.current_total_price).toLocaleString() : 0}원</div>
          </div>
        ):(
