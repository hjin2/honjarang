...
export default function PurchaseDetail() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API.PURCHASES}/${id}/applicants`, { headers })
      .then((res) => {
        // console.log(res.data)
        if (res.data) {
  ...
  const deletePurchase = () => {
    axios
      .delete(`${API.PURCHASES}/${id}`, { headers })
      .then((res) => {
        console.log(res);
        navigate('/market');
      })
  ...
  const CancelPurchase = () => {
    axios
      .delete(`${API.PURCHASES}/${id}/applicants`, { headers })
      .then((res) => {
        console.log(res);
        setIsPurchase(false);
      })
  ...
  const handleCheck = () => {
    console.log(id);
    axios
      .put(`${API.PURCHASES}/${id}/receive`, [], { headers })
      .then((res) => {
        console.log(res);
  ...
  const enter = () => {
    navigate(`/chatting/${detail.chat_room_id}`, {
      state: { title: `${detail.product_name} 공동구매 채팅방` },
    });
  };
    ...
    <div className="w-6/12 mx-auto mt-5">
      <div className="border rounded-lg p-8 space-y-5 ">
        <div className="flex justify-between">
          <div className="flex flex-row space-x-4">
            <div className="font-bold text-3xl items-end">
              {detail.product_name}
            </div>
            <div className="text-main5 mt-3">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <div className="space-x-2">
              <button
                onClick={() => handleTabClick('purchase_detail_product')}
                className={`mr-2 ${
              ...
              <button
                onClick={() => handleTabClick('purchase_detail_place')}
                className={`${
      ...
      <div className="border rounded-lg my-3 p-5 mx-auto items-center text-center">
          ...
          <div>
            {detail.current_person_count !== detail.target_person_count ? (
              <div className="space-y-3 mb-3">
                <div className="text-main5">
                  마감까지 남은 시간 : {days}일 {hours}시간 {minutes}분{' '}
                  {seconds}초
                </div>
                <div className="text-main2">
                  목표까지{' '}
                  {detail.target_person_count - detail.current_person_count}명
                </div>
              </div>
            ) : (
            ...
            {isPurchase ? (
              <button
                ...
                className="main5-full-button w-40"
              >
                공동구매 취소
              </button>
            ) : (
              <button
                ...
                className="main1-full-button w-40"
              >
                공동구매 신청
              </button>
            )}
        ...
        <PurchaseApply
          ...
          deliveryCharge={
            Math.floor(
              detail.delivery_charge / detail.target_person_count / 10,
            ) * 10
          }
          purchaseId={id}
