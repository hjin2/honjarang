...
export default function TransactionDetail() {
    ...
    axios
      ...
      .then((res) => {
        ...
        navigate(`/chatting/${res.data}`, {
          state: { title: detail.seller_nickname },
        });
      })
  ...
  const handelTransaction = () => {
    axios
      .delete(`${API.SECONDHAND}/${id}`, { headers })
      .then((res) => {
        console.log(res);
  ...
  const buy = () => {
    axios
      .put(`${API.SECONDHAND}/${id}/buy`, [], { headers })
      .then((res) => {
        console.log(res);
        setIsSell(true);
      })
  ...
  const check = () => {
    axios
      .put(`${API.SECONDHAND}/${id}/check`, [], { headers })
      .then((res) => {
        console.log(res);
        setIsFinish(true);
      })
    ...
    <div className="w-6/12 mx-auto mt-5 border rounded-lg p-5">
        ...
        <div className="flex">
          ...
          {isClick && isWriter ? (
            <div className="absolute border-2 rounded-lg bg-white text-center space-y-2 p-2">
              <div className="cursor-pointer" onClick={editTransaction}>
                수정
              </div>
              <div className="cursor-pointer" onClick={handelTransaction}>
                삭제
              </div>
            </div>
          ) : null}
      ...
      <div className="mt-3">
        {detail.transaction_image ? (
          <img src={`${detail.transaction_image}`} alt="상품이미지" />
        ) : null}
        <div className="my-10">{detail.content}</div>
        {!isWriter && (
          <div>
            {isSell && isBuyer && !isFinish && (
              <button className="main1-full-button w-full" onClick={check}>
                수령확인
              </button>
            )}
            {!isSell && (
              <button className="main1-full-button w-full" onClick={buy}>
                구매
              </button>
            )}
          </div>
